window.onload = function () {
						...
						fetch(`${window.API_URL}/users/emailSending/${email}`, {
							...
							.then(data => {
								if (data.recoveryCode !== null) {
									...
									document.getElementById('ButtonRecovery').style.display =
										'none'

									// Отображаем поле для ввода кода
									document.getElementById('recoveryCode').style.display =
										'block'
									document
										...
										.addEventListener('input', function () {
											...
											if (enteredCode.length === 6) {
												...
												if (enteredCode === data.recoveryCode.toString()) {
													...
													alert(
														'Неверный код восстановления. Попробуйте еще раз.'
													)
												}
											}
										})
	...
	passwordButton.addEventListener('click', function (e) {
		...
		fetch(
			`${window.API_URL}/visitor/updatepasswordVisitor/${id}`,
			requestOptions
		)
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		document
			.querySelector('.nav-container1 .menu-items li:nth-child(2) a')
		...
		document
			.querySelector('.nav-container1 .menu-items li:nth-child(3) a')
			.addEventListener('click', function (event) {
				event.preventDefault()
				handleMenuItemClick()
